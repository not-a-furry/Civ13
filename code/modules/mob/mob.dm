		...
		if (client.status_tabs && statpanel("Status") && ticker)
			...
			if (map && !map.civilizations)
				...
				for (var/faction in map.faction_organization)
					if (!list(BRITISH, PIRATES, INDIANS, PORTUGUESE, SPANISH, FRENCH, DUTCH, CIVILIAN, ROMAN, ISRAELITE, GREEK, ARAB, JAPANESE, RUSSIAN, GERMAN, AMERICAN, VIETNAMESE).Find(faction))
						continue
